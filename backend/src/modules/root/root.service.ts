...
export class RootService {
    ...
    public async serveSubscriptionPage(
        ...
        try {
            ...
            // Try whitelist routing before standard Remnawave subscription
            const whitelistServed = await this.whitelistService.tryServeWhitelist(
                clientIp,
                shortUuidLocal,
                req,
                res,
    ...
    private async returnWebpage(
        ...
        try {
            ...
            // Determine subscription tier for the page title
            const isWhitelist = await this.whitelistService.isWhitelistActive(shortUuid);
            const titleSuffix = isWhitelist ? ' - Подписка VIP' : ' - Подписка Standard';

            res.render('index', {
                metaTitle: baseSettings.metaTitle + titleSuffix,
